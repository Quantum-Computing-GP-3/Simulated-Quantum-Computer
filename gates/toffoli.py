...
class Toffoli(Gate):
    ...
    def acts_on(self, Reg_obj, q):
        ...
        if len(q) != 3 or len(q) != len(set(q)):
            raise ValueError(
                'Error: gate expects 3 nonidentical qubit arguments')

        c1 = q[0]  # control position
        c2 = q[1]  # control position2
        t = q[2]  # target position

        # sorting the list with 3 entries to fix the conditions for iteration
        # according to qubit order
        qprime = np.sort(q)
        cond1 = 2 ** qprime[0]
        cond2 = 2 ** qprime[1]
        cond3 = 2 ** qprime[2]
        ...
        # iteration over all states for which both control qubits are 1, then
        # define action on target
        while i < Reg_obj.N - 1:
            for _ in range(int(between2)):
                for _ in range(int(between1)):
                    for _ in range(cond1):
                        a = Reg_obj.Reg[i]
                        # action on t is switch states i and i+2**t
                        b = Reg_obj.Reg[i + 2 ** t]
                        Reg_obj.Reg[i] = b
                        Reg_obj.Reg[i + 2 ** t] = a
                        i += 1
